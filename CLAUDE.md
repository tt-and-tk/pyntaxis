...
```
期待値は `test/sv_ans/`・`test/bin_ans/`(実行ファイルと同じ形式のバイナリ)・`test/err_ans/`・`test/bin_err_ans/`(出力されるエラーメッセージ．異常系の入力と同名で拡張子`.txt`) にある．エラーメッセージの期待値は入力の行をタブ文字ごと引用することがあるため，手で書かずアセンブラの出力から作り，入力の先頭コメントに書いた意図と合っているかを確かめる．
