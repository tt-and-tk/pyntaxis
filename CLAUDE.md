...
`test/asm/`(`01.pt`〜`12.pt`)にある正常系テストケースを`test/test.py`で一括実行できる。call/retの多段・再帰呼び出し，P/S/F/M/IO各系命令，局所ラベルの前方/後方ジャンプなど，本CLAUDE.mdに記載の仕様を一通り網羅する。各ファイルの内容はソース(コメント)を参照。
