`include "rom.svh"
`include "machine.svh"

module rom_sv(
    input logic clk,
    rom_read_if.slave rom_read
    );
    import machine_p::*;

    localparam integer ROM_SIZE = 17;

    (* rom_style = "block" *) machine_t machines[0:ROM_SIZE - 1] = {
        sll(1, 2, 3, 0),
        srl(1, 2, 3, 0),
        sla(1, 2, 3, 0),
        sra(1, 2, 3, 0),
        eq(1, 2, 33'h1_0000_0000 + 12),
        ne(1, 2, 33'h1_0000_0000 + 11),
        lt(1, 2, 33'h1_0000_0000 + 10),
        gt(1, 2, 33'h1_0000_0000 + 9),
        elt(1, 2, 33'h1_0000_0000 + 8),
        egt(1, 2, 33'h1_0000_0000 + 32'hfffffff7),
        ltu(1, 2, 33'h1_0000_0000 + 6),
        gtu(1, 2, 33'h1_0000_0000 + 5),
        eltu(1, 2, 33'h1_0000_0000 + 4),
        egtu(1, 2, 33'h1_0000_0000 + 32'hfffffff3),
        jmp(0, 33'h1_0000_0000 + 16),
        jmp(0, 33'h1_0000_0000 + 0),
        jmp(0, 33'h1_0000_0000 + 16)
    };

    always_ff @(posedge clk) begin
        rom_read.valid <= (rom_read.pc < ROM_SIZE);

        if (rom_read.pc < ROM_SIZE) begin
            rom_read.machine <= machines[rom_read.pc];
        end else begin
            rom_read.machine <= nop();
        end
    end

endmodule
